...
btnPop?.addEventListener("click", () => {
  const innerWin = globalThis.open(
    "pop.html",
    "pop",
  ...
  win.addEventListener("load", () => {
    ...
    const tpPopClock = <TPClock> innerWin.document.querySelector("#timeTimer");
...
prgSpeed.addEventListener("wheel", (e) => {
  ...
  const speed = prgSpeed.value * 10 - 500;
...
rngScale.addEventListener("sl-input", () => {
  const msg: messageWindow = {
    method: "textScale",
    args: [rngScale.value],
  };
